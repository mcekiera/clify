...
const { InvalidArgsError, withInvalidArgsErrorHandling } = require('./utils/errors');
...
class Cli {
  ...
  decorateWithInternalActions(structure) {
    return {
      ...structure,
      _: {
        previewFiles: this.preview.bind(this, { listFunc: this.actions.resolve.listFiles, getFunc: this.actions.resolve.load }),
        previewStorage: this.preview.bind(this, { listFunc: this.actions.resolve.listKept, getFunc: this.actions.resolve.get }),
      },
    };
  }
  ...
  async runThread() {
    ...
    const args = await withInvalidArgsErrorHandling(async () => {
      const input = await this.collectArgsInput(func, path);
      return this.prepareArgs(input);
    });
    ...
    switch (value) {
      case Actions.USE:
        ...
        if (choices.length) {
          ...
          throw new InvalidArgsError('no variables to $use');
        }
        break;
      case Actions.LOAD:
        ...
        if (choices.length) {
          ...
          throw new InvalidArgsError('no files to $load');
        }
