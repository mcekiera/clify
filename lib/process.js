...
const {
  preparePostResultMessage, logResultMessage, logActionResult, logSeparator, logNonExistingPath, logChangedResultValue,
...
async function runOnResult(actions, choice, data) {
  ...
  const select = await actions.runAutoComplete('after', 'With result:', 10, localChoice);

  if (select === '$extract') {
    let isValid = false;
    while (!isValid) {
      const path = await actions.runInput('path', 'Enter path to value:');

      if (has(result, path)) {
        const extracted = get(result, path);
        isValid = true;
        logChangedResultValue(extracted);
        return runOnResult(actions, choice, extracted);
      }
      logNonExistingPath(path);
    }
  } else if (actions.after[select]) {
...
async function prepareArgs(actions, args, structure) {
  const obj = { ...args };
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (value === '$func') {
      const funcResult = await runThread(actions, structure);
      obj[key] = await runOnResult(actions, ['$done'], funcResult);
    }
  }
...
async function runThread(actions, structure) {
  ...
  const args = await prepareArgs(actions, input, structure);
...
async function runCli(structure, actions = null) {
  ...
  try {
    ...
    await runOnResult(passActions, choice, result);
  } catch (error) {
    if (!error) {
      process.exit(0);
    }
